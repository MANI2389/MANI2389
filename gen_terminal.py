...
Generates TWO SVG files, side-by-side in the README:
  ...
  terminal-card.svg  (840 × 875)
    ...
    • macOS chrome:  MANI2389@github: ~$ ./portrait.sh
    • Footer:  MANI2389@github:~$ whoami  Thangamani A

  info-card.svg  (480 × 420)
    • neofetch-style info card, matching Avi's reference exactly
    • Staggered slide-up + fade-in per row
    • macOS chrome:  MANI2389@github: ~$ neofetch
...
PHOTO_PATH = os.path.join(OUT_DIR, "assets", "profile1.jpg")

print("[..] Loading local profile photo …")
try:
    img = Image.open(PHOTO_PATH).convert("L")
except Exception as e:
    sys.exit(f"[ERR] Profile photo unavailable: {e}")

# Density ramp: space = bright pixel, @ = dark pixel
ASCII_CHARS = "  `.-':=+*csS%#@"
ART_W, ART_H = 100, 53

# Crop around the face so the ASCII portrait keeps recognizable features.
img = img.crop((300, 20, 950, 720))
...
INFO_ROWS = [
    ...
    {"type": "field",    "label": "Role",     "value": "AI Engineer | Data Analyst"},
    {"type": "field",    "label": "Focus",    "value": "AI/ML and Data Analytics"},
    {"type": "field",    "label": "College",  "value": "B.Tech, Kavery Engg. College"},
    {"type": "field",    "label": "Location", "value": "Pennagaram, Tamil Nadu"},
    # Stack section
    ...
    {"type": "bullet",   "value": "6 public repositories"},
    {"type": "bullet",   "value": "1 GitHub star"},
]
...
for row in INFO_ROWS:
    ...
    if row["type"] == "header":
        # "MANI2389@github" bold header + horizontal rule
        ulen = len(USERNAME)
